...
class TrainCIFARCluster(object):
    ...
    def _load_CIFAR(self, train=True):
        ...
        if train:
            cifar_dataset = datasets.CIFAR10(root='./data', train=True, download=True, transform=transforms)
        else:
            cifar_dataset = datasets.CIFAR10(root='./data', train=False, download=True, transform=transforms)

        dl = DataLoader(cifar_dataset)

        # CIFAR-10 images are already 3x32x32, so no need to unsqueeze
        # X = dl.dataset.data  # shape: (N, 32, 32, 3), numpy array
        # y = dl.dataset.targets  # list of labels
        ...
        for img, label in cifar_dataset:
            X.append(img)
            y.append(label)
        X = torch.stack(X)  # shape: (N, 3, 24, 24)
        y = torch.tensor(y)

        # Convert X to torch tensor and permute to (N, 3, 32, 32)
        # X = torch.tensor(X).permute(0, 3, 1, 2).float() / 255.0  # normalize to 0~1
        # y = torch.tensor(y)
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        # conv2: [5x5 conv, in_channels=64, out_channels=64]
        ...
        self.fc1 = nn.Linear(in_features=64*6*6, out_features=384)
        self.fc2 = nn.Linear(in_features=384, out_features=192)
        self.fc3 = nn.Linear(in_features=192, out_features=10)
