    ...
    def summarize(self, force = False):
        ...
        if os.path.exists(results_fname) and not force:
            ...
            for t_i, task in enumerate(self.tasks):
                ...
                result_fname5 = task.procs[4].result_fname
                ...
                with open(result_fname5, 'rb') as f:
                    try:
                        res = pickle.load(f)
                        last_acc = res[-1]['test']['acc']
                        key = tuple(cfg.values())
        ...
        for cfg in cfgs2:
            ...
            for d_i in self.cfg['data_seed']:
                ...
                last_acc4 = results[4][key2]
                last_acc4s.append(last_acc4)
            ...
            print(cfg,
                f' IFCA {np.mean(last_acc0s):.5f} +- {np.std(last_acc0s):.5f} Global {np.mean(last_acc1s):.5f} +- {np.std(last_acc1s):.5f} Local {np.mean(last_acc2s):.5f} +- {np.std(last_acc2s):.5f} DFCA {np.mean(last_acc4s):.5f} +- {np.std(last_acc4s):.5f} ds {len(self.cfg["data_seed"])}')
...
class MyTask(PRTask):
    def __init__(self, cfg):
        ...
        proc1 = PRProcess(
            command = [
                "python", "-u", "train_cluster_emnist.py",
            ...
            cleanup_fnames =[],
            env = env0,
            stdout_prefix = "train_cluster_emnist"
            )

        proc2 = PRProcess(
            command = [
                "python", "-u", "train_cluster_emnist_single.py",
                "--config-override", json.dumps(cfg)
                ],
            output_dir = project_dir,
            ...
            cleanup_fnames =[],
            env = env0,
            stdout_prefix = "train_cluster_emnist_single"
            )

        proc3 = PRProcess(
            command = [
                "python", "-u", "train_cluster_emnist_local.py",
                "--config-override", json.dumps(cfg)
                ],
            output_dir = project_dir,
            ...
            cleanup_fnames =[],
            env = env0,
            stdout_prefix = "train_cluster_emnist_local"
            )
        ...
        proc4 = PRProcess(
            ...
            cleanup_fnames =[],
            env = env0,
            stdout_prefix = "train_cluster_emnist_ckpt_local"
            )
        
        proc5 = PRProcess(
            command = [
                "python", "-u", "train_cluster_emnist_dfca-gi.py",
                "--config-override", json.dumps(cfg)
                ],
            output_dir = project_dir,
            result_fname = 'results_difca.pickle',
            cleanup_fnames =[],
            env = env0,
            stdout_prefix = "train_cluster_emnist_dfca-gi"
            )

        self.procs = [proc1, proc2, proc3, proc4, proc5]
