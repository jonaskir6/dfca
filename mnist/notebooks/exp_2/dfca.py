...
#                self.save_checkpoint()
    ...
    def dec_param_update(self, cluster_assign):
        ...
        # calculate the maximum number of possible exchange partners for m_i (capped at 30)
        if num_clients > 800:
            max = 50
        else:
            max = 15
    ...
    #         # pick a random number of exchnage partners for m_i
    #         m_i_partners = random.sample([i for i in client_indices if i != m_i], torch.randint(1, threshold, (1,)))
    #         avg_sc += len(m_i_partners)
    #         m_i_cluster = cluster_assign[m_i]
    #         # client m_i averages parameters with all selected clients 
    #         for m_j in m_i_partners:
    #             m_j_cluster = cluster_assign[m_j]
    #             m_j_params = dict(self.models[m_j][m_j_cluster].named_parameters())

    #             # choose weighting factor, 0.4 for different clusters, 0.5 for same cluster
    #             alpha = 0.5 if m_i_cluster == m_j_cluster else 0.4 
