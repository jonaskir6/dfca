...
class MyProcessRunner(ProcessRunner):
    def summarize(self, force = False):
        ...
        for cfg in cfgs2:
            ...
            print(cfg,
                f' IFCA {np.mean(last_acc0s):.5f} +- {np.std(last_acc0s):.5f} Global {np.mean(last_acc1s):.5f} +- {np.std(last_acc1s):.5f} Local {np.mean(last_acc2s):.5f} +- {np.std(last_acc2s):.5f} DIFCA {np.mean(last_acc4s):.5f} +- {np.std(last_acc4s):.5f} ds {len(self.cfg["data_seed"])}')
...
class MyTask(PRTask):
    def __init__(self, cfg):
        ...
        proc5 = PRProcess(
            command = [
                "python", "-u", "train_cluster_mnist_difca.py",
                "--config-override", json.dumps(cfg)
                ],
            output_dir = project_dir,
            ...
            cleanup_fnames =[],
            env = env0,
            stdout_prefix = "train_cluster_mnist_difca"
            )
