...
class TrainMNISTCluster(object):
    ...
    def get_inference_stats(self, train = True):
        ...
        for m_i in range(m):
            ...
            machine_losses.remove(min_p_i)
            sec_min_p_i = np.argmin(machine_losses)
            rho = (min_p_i + 0.1(abs(min_p_i - sec_min_p_i)))
            cnt = 0
            while cnt < cfg['k\'']:
                for p_i in range(p):
                    if machine_losses[p_i] < rho:
                        cluster_assign.append(p_i)
                        cnt += 1
                        break
    ...
    def load_data(self, m_i, train=True):
        ...
        # additional transformation with a 20% chance
        if random.random() < 0.2:  # 20% chance
            additional_rotation = random.choice([1, 2, 3])
            ...
            X_batch3 = torch.cat((X_batch3, X_batch_additional_rot), dim=0)
    ...
    def dec_mc_param_update(self, local_models, global_model):
